...
SHELL       = /bin/sh
SWIG_LIB_INSTALL = ${datarootdir}/swig/3.0.11
BIN_DIR     = ${exec_prefix}/bin
ENABLE_CCACHE = 1
TARGET_NOEXE= swig
TARGET      = $(TARGET_NOEXE).exe
SOURCE      = Source
CCACHE      = CCache
DOCS        = Doc/Manual
HAVE_CXX11_COMPILER = 
...
delphi_examples         :=$(shell sed '/^\#/d' $(srcdir)/Examples/delphi/check.list)
...
INSTALL         = /swig/swig-master/Tools/config/install-sh -c
INSTALL_DATA    = ${INSTALL} -m 644
INSTALL_PROGRAM = ${INSTALL} -m 755
MKINSTDIRS      = /swig/swig-master/Tools/config/install-sh -m 0755 -d
...
install-main:
	...
	@echo "Installing $(DESTDIR)$(BIN_DIR)/`echo $(TARGET_NOEXE) | sed '$(transform)'`.exe"
	@$(INSTALL_PROGRAM) $(TARGET) $(DESTDIR)$(BIN_DIR)/`echo $(TARGET_NOEXE) | sed '$(transform)'`.exe

lib-languages = gcj typemaps tcl perl5 python guile java mzscheme ruby php php5 ocaml octave \
	pike chicken csharp modula3 allegrocl clisp lua cffi uffi r go d delphi javascript javascript/jsc \
...
uninstall-main:
	@echo "Uninstalling SWIG executable $(DESTDIR)$(BIN_DIR)/`echo $(TARGET_NOEXE) | sed '$(transform)'`.exe"
	rm -f $(DESTDIR)$(BIN_DIR)/`echo $(TARGET_NOEXE) | sed '$(transform)'`.exe
...
dist:
	...
	@echo "Tools/mkdist.py 3.0.11 master"
	@false
